...
def compute_fid(real_params, gen_params):
    print(real_params.shape)
    print(gen_params.shape)
    mu_real = np.mean(real_params, axis=0)
    cov_real = np.cov(real_params, rowvar=False)
    mu_gen = np.mean(gen_params, axis=0)
    cov_gen = np.cov(gen_params, rowvar=False)
    ...
    # 计算 FID
    fid = mean_diff + np.trace(cov_real + cov_gen - 2 * cov_sqrt)
    return fid