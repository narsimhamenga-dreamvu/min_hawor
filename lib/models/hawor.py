...
class HAWOR(pl.LightningModule):
    ...
    def inference(self, imgfiles, boxes, img_focal, img_center, device='cuda', do_flip=False,
                  batch_size=1, num_workers=4, amp_dtype=None):
        ...
        # The space-time / motion modules operate on fixed temporal windows of T=16
        # frames (see the `t=16` einops rearranges in forward_step). Previously this
        # ran exactly ONE window per forward pass (batch dim = 1) while loading crops
        # in a serial python loop. We now:
        #   (1) load crops with a multi-worker DataLoader (the real win: crop loading
        #       was serial and blocked the GPU),
        #   (2) optionally run the forward in mixed precision (amp_dtype),
        #   (3) allow batching `batch_size` windows per forward -- but note the ViT-H
        #       backbone is already saturated by the 16 crops in a single window, so
        #       batch_size > 1 does NOT speed up the GPU; it is kept configurable only
        #       (smaller batches actually pipeline better against the loader).
        # Batch boundaries land on window boundaries because `batch_size * T` is a
        # multiple of T, so only the trailing partial window is padded -> numerically
        # identical to the old per-window path.
        T = 16
        loader = torch.utils.data.DataLoader(
            db, batch_size=batch_size * T, shuffle=False,
        ...
        for batch in tqdm(loader):
            n_frames = batch['img'].shape[0]

            # pad the trailing partial window up to a whole multiple of T by repeating
            # the last frame (mirrors the old "padding to 16" of the final chunk)
            pad = (-n_frames) % T
            if pad:
                batch = {k: torch.cat([v, v[-1:].expand(pad, *v.shape[1:])], dim=0)
                         for k, v in batch.items() if type(v) == torch.Tensor}
            n_windows = (n_frames + pad) // T

            with torch.no_grad():
                # (n_windows*T, ...) -> (n_windows, T, ...) so forward_step's
                # `(b t) -> b t` rearranges see b=n_windows temporal windows
                batch = {k: v.to(device, non_blocking=True).reshape(n_windows, T, *v.shape[1:])
                         for k, v in batch.items() if type(v) == torch.Tensor}
            ...
            # out tensors are flattened to (n_windows*T, ...); drop the pad frames
            # and cast back to fp32 (autocast may leave some outputs in amp_dtype)
            pred_cam.append(out['pred_cam'][:n_frames].float().cpu())
            pred_pose.append(out['pred_pose'][:n_frames].float().cpu())
            pred_shape.append(out['pred_shape'][:n_frames].float().cpu())
            pred_rotmat.append(out['pred_rotmat'][:n_frames].float().cpu())
            pred_trans.append(out['trans_full'][:n_frames].float().cpu())
