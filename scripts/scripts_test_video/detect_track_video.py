...
from lib.pipeline.tools import detect_track, detect_track_ego
...
def detect_track_video(args):
    ...
    if getattr(args, 'ego_filter', False):
        print("Using Molmo ego-hand filtering (assist-YOLO)")
        boxes_, tracks_ = detect_track_ego(
            imgfiles, thresh=0.2,
            sample_every=getattr(args, 'molmo_sample_every', 30),
            min_votes=getattr(args, 'molmo_min_votes', 2),
            molmo_batch=getattr(args, 'molmo_batch', 1))
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ego_filter", action="store_true", help="Molmo ego-hand filtering")
    parser.add_argument("--molmo_sample_every", type=int, default=30)
    parser.add_argument("--molmo_min_votes", type=int, default=2)
    parser.add_argument("--molmo_batch", type=int, default=1)
