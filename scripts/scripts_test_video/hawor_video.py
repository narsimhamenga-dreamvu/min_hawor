...
def hawor_motion_estimation(args, start_idx, end_idx, seq_folder):
    ...
    img_center = [img.shape[1] / 2, img.shape[0] / 2]# w/2, h/2  (default: image center)
    # allow overriding the principal point (e.g. real ppx/ppy from intrinsics),
    # threaded into get_trans / projection / bbox_est via img_center
    _cx = getattr(args, 'img_cx', None); _cy = getattr(args, 'img_cy', None)
    if _cx is not None: img_center[0] = float(_cx)
    if _cy is not None: img_center[1] = float(_cy)
    print(f"img_center (principal point) = {img_center}")
    H, W = img.shape[:2]
    model_masks = np.zeros((len(imgfiles), H, W), dtype=bool)  # bool, not float64: at 1080p x 1000s of frames float64 = tens of GB -> OOM
    ...
    for idx in tid:
        ...
        for frame_ck, boxes_ck in zip(frame_chunks, boxes_chunks):
            ...
            _prec = getattr(args, 'motion_precision', 'fp32')
            _amp_dtype = {'fp32': None, 'bf16': torch.bfloat16, 'fp16': torch.float16}[_prec]
            results = model.inference(img_ck, boxes_ck, img_focal=img_focal, img_center=img_center, do_flip=do_flip,
                                      batch_size=getattr(args, 'motion_batch', 1),
                                      num_workers=getattr(args, 'motion_workers', 4),
            ...
            for img_i, _ in enumerate(img_ck):
                ...
                model_masks[frame_ck[img_i]] |= np.asarray(mask) > 0
